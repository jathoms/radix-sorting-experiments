...
pub fn paradis_sort_i32_new_uninit(values: &[i32]) -> Vec<i32> {
    let mut output = uninit_vec(values.len());
    let workers = rayon::current_num_threads();
    paradis_sort_to_uninit_output_recursive(
        values,
        &mut output,
        24,
        workers,
        COMPARISON_FALLBACK_THRESHOLD,
    );

    unsafe { assume_init_vec(output) }
}
...
fn paradis_sort_to_uninit_output_recursive(
    values: &[i32],
    ...
    if values.len() <= fallback_threshold {
        copy_to_uninit(values, output);
        let output = unsafe { assume_init_slice_mut(output) };
    ...
    if scan.reversed {
        write_reversed_to_uninit(values, output);
        return;
    }
    if is_nearly_sorted(values.len(), scan.adjacent_inversions) {
        copy_to_uninit(values, output);
    ...
    partition_to_uninit_output(values, output, &bucket_starts, &scan, shift);
...
fn partition_to_uninit_output(
    ...
    let ptr = UninitSendPtr(output.as_mut_ptr());
    values
...
fn copy_to_uninit(values: &[i32], output: &mut [MaybeUninit<i32>]) {
    assert_eq!(values.len(), output.len());
    unsafe {
        std::ptr::copy_nonoverlapping(
            values.as_ptr(),
            output.as_mut_ptr().cast::<i32>(),
            values.len(),
        )
    };
}

fn write_reversed_to_uninit(values: &[i32], output: &mut [MaybeUninit<i32>]) {
    assert_eq!(values.len(), output.len());
...
unsafe fn assume_init_slice_mut(slice: &mut [MaybeUninit<i32>]) -> &mut [i32] {
    unsafe { std::slice::from_raw_parts_mut(slice.as_mut_ptr().cast::<i32>(), slice.len()) }
}

unsafe fn assume_init_vec(vec: Vec<MaybeUninit<i32>>) -> Vec<i32> {
    let mut vec = ManuallyDrop::new(vec);
    unsafe { Vec::from_raw_parts(vec.as_mut_ptr().cast::<i32>(), vec.len(), vec.capacity()) }
}
...
#[derive(Clone, Copy)]
struct UninitSendPtr(*mut MaybeUninit<i32>);

unsafe impl Send for UninitSendPtr {}
unsafe impl Sync for UninitSendPtr {}

impl UninitSendPtr {
    unsafe fn write(self, index: usize, value: i32) {
        unsafe { self.0.add(index).write(MaybeUninit::new(value)) };
    }
}
...
mod tests {
    ...
    #[test]
    fn test_paradis_sort_i32_new_uninit() {
        assert_paradis_sorts_uninit("random", Distribution::Random.values(500_000));
    }

    #[test]
    fn test_paradis_uninit_patterns() {
        assert_paradis_sorts_uninit("small", Distribution::Random.values(1_000));
        assert_paradis_sorts_uninit("sorted", (0..100_000).collect());
        assert_paradis_sorts_uninit("reversed", (0..100_000).rev().collect());
        assert_paradis_sorts_uninit("few_unique", (0..100_000).map(|i| (i % 128) - 64).collect());
        assert_paradis_sorts_uninit(
            "same_high_byte",
            (0..100_000)
    ...
    fn assert_paradis_sorts_uninit(name: &str, values: Vec<i32>) {
        let mut expected = values.clone();

        expected.sort_unstable();
        let actual = paradis_sort_i32_new_uninit(&values);
